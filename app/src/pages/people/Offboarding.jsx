...
export default function Offboarding() {
    ...
    const loadData = async () => {
      try {
        ...
        const [employeeRes, templatesData] = await Promise.all([
          fetch('/api/employees?status=offboarded'),
          fetchTemplates(),
        ]);
        ...
        setTemplates(templatesData?.filter(t => t.type === 'offboarding') || []);
        setError(null);
      } catch (err) {
      ...
      try {
        ...
        if (checklists && checklists.length > 0) {
          ...
          const progressRes = await fetch(
            `/api/offboarding/checklists/${checklist.id}/progress`
          );
          const { data: progressData } = await progressRes.json();
          setProgress(progressData);

          // Set up run for detailed view
          setSelectedRun({
            id: checklist.id,
  ...
  const handleMarkComplete = async (itemId) => {
    ...
    try {
      ...
      const progressRes = await fetch(
        `/api/offboarding/checklists/${selectedChecklist.id}/progress`
      );
      const { data: progressData } = await progressRes.json();
      setProgress(progressData);
    } catch (err) {
      ...
      <div className="p-8">
        ...
        <OnboardingRun
          ...
          onUpdateTask={async (taskId, newStatus) => {
            const itemId = selectedChecklist.items.find(
              i => i.id === taskId
            )?.id;
            if (itemId) {
              await markItemComplete(selectedChecklist.id, itemId, 'current-user');
          ...
          onBulkAction={async (action) => {
            if (action === 'mark-all-complete') {
              ...
              for (const item of items) {
                if (!item.completed) {
                  await markItemComplete(selectedChecklist.id, item.id, 'current-user');
                }
              }
              const detail = await getChecklistDetail(selectedChecklist.id);
              setSelectedChecklist(detail);
  ...
  const groupItemsByPhase = (items) => {
    ...
    items.forEach((item) => {
      ...
      if (daysFromNow <= 0) phases['Pre-Offboard'].push(item);
    ...
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-slate-600">
          Track employee departures, access removal, and device recovery
        </p>
      </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredEmployees.length === 0 ? (
                <div className="p-6 text-center">
                  <p className="text-slate-500">No departing employees</p>
                </div>
              ) : (
                    ...
                    <button
                      onClick={async () => {
                        if (selectedTemplateId) {
                          try {
                            await fetch('/api/offboarding/checklists', {
                              method: 'POST',
